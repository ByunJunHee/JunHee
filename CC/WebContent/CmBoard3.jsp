		...
		<div class="board2">
			<table border="1">
				<tr>
				 ...
				 <%for (CmCarGalleryVo cgv : alist) { %>
				<tr>
					<td><%=cgv.getCgIdx()%></td>
					<td><a href="<%=request.getContextPath()%>/Board/CmBoard3Contents.do?CgIdx=<%=cgv.getCgIdx()%>"><%=cgv.getCgTitle()%></a></td>
					<td><%=m_id %></td>
			...
			<br>
			<div class="button">
				<a href="<%=request.getContextPath() %>/Board/CmBoard3Write.do"><button type="button">글쓰기</button></a>
			</div>
			<br>
			<div class="pagingArea">
				<table border=0 style="text-align:left;width:800px;height:80px">
					<tr>
						<td width="50px">
						<% if(pm.isPrev() == true) { %>
						<a href="<%=request.getContextPath()%>/Board/CmBoard3.do?page=<%=pm.getStartPage()-1%>">◀</a>
						<% } %>
						</td>
						<td>
						<%
						...
						%>
						&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/Board/CmBoard3.do?page=<%=i%>&keyword=<%=pm.encoding(pm.getScri().getKeyword())%>"><%=i%></a>
						<%} %>
						</td>
						<td width="50px">
						<%if (pm.isNext() == true){ %>
						<a href="<%=request.getContextPath()%>/Board/CmBoard3.do?page=<%=pm.getEndPage()+1%>&keyword=<%=pm.encoding(pm.getScri().getKeyword())%>">
						▶</a>
						<%} %>
						</td>
					</tr>
